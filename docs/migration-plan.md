...
| 8 | Realtime | ✅ **Concluída para o repo React** (staff) — `onSnapshot` trocado por `supabase.channel(...).on('postgres_changes', ...)` em `src/lib/queue.ts`, com re-fetch da query filtrada a cada evento (fonte da verdade é sempre a query, o canal só decide *quando*). Falta o lado Flutter (parte da Fase 10 abaixo) | Fase 4, 5 |
...
| 10 | Flutter/React | 🔶 **Metade feita** — `fila-certa-staff` (React) migrado por completo: `src/supabase.ts` novo, `AuthContext.tsx`, `src/lib/queue.ts` e `PublicDisplay.tsx` (sessão anónima) a usar o SDK Supabase; `AgentScreen.tsx` actualizado às novas assinaturas RPC (o servidor escolhe a próxima senha, já não o cliente). Testado com `tsc -b`, `npm run build` e um teste funcional real contra o projecto (view `counters_with_agent`, filtro de transferência, leitura anónima) — publicado em `filacerta-staff.web.app`. Firebase (`src/firebase.ts`) fica no repo, sem ser importado por ninguém, até à Fase 15. Falta o lado Flutter (`projectogestaodefilas`) | Fases 6, 8, 9 |
