...
| 8 | Realtime | ✅ **Concluída nos dois repos** — `onSnapshot`/`.snapshots()` trocados por `supabase.channel(...).on('postgres_changes'/'onPostgresChanges', ...)` em `src/lib/queue.ts` (React) e `ticket_service.dart`/`app_stores.dart` (Flutter), sempre com re-fetch da query filtrada a cada evento (a fonte da verdade é sempre a query, o canal só decide *quando*) | Fase 4, 5 |
