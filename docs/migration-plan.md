...
| 6 | Auth | ✅ **Concluída** — as 12 contas de staff recriadas em Supabase Auth (mesmos emails, password partilhada -- ver `docs/security-credentials.md` para o estado actual, a password original ficou comprometida por ter sido commitada e foi corrigida em 2026-09-12), tabela `staff` populada, e `institutions`/`branches`/`counters` trazidos do Firestore como dados de referência (pré-requisito de `staff` via FK). Verificado por `JOIN` entre `staff` e `auth.users`: as 12 linhas batem certo. Contas de cliente de teste ainda por fazer (ver nota) | Fase 4 |
...
| 14 | Produção | ✅ **Concluída, incluindo hosting** — os dois frontends falam só com o Supabase desde a Fase 10. Actualização de 2026-09-07: por pedido explícito do utilizador ("proibido manter Firebase para qualquer finalidade, incluindo hosting"), o alojamento também saiu do Firebase Hosting — agora em GitHub Pages: `https://paulinoquicassa.github.io/Fila-certa/` (staff) e `https://paulinoquicassa.github.io/DevSYNOVAR/` (cliente). Ver `README.md` para o processo de deploy (branch `gh-pages` órfã por repo) | Fase 13 |
| 15 | Desligar Firebase | 🔶 **Código e hosting concluídos; projecto Cloud pendente** — código 100% sem Firebase (verificação global, 2026-09-07), hosting migrado para GitHub Pages. **Falta só**: eliminar o projecto Firebase Cloud (`filacerta-d74f0`) em si — tentado via `gcloud projects delete` com a credencial de serviço (Admin SDK) disponível, recusado por falta de permissão (`resourcemanager.projects.delete` exige papel de Owner, que esta credencial não tem). Requer uma acção manual do utilizador — ver README.md/relatório para o comando exacto. Backup do Firestore feito na Fase 0 continua disponível até essa eliminação acontecer | Fase 14 estável |
...
teste partilhadas, sem nenhum utilizador real. **Actualização
2026-09-12**: essa simplicidade foi exactamente o que permitiu a
password de staff ficar commitada e comprometida -- ver
`docs/security-credentials.md`. A password deixou de estar hardcoded
em qualquer ficheiro; decidir por variável de ambiente a cada vez.
